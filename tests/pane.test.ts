...
describe("Pane component", () => {
  ...
  it("no longer renders a gear/settings button on the pane header (moved to the Panes modal in PR-32)", async () => {
    const { container } = await mountPane({ label: "Editor" });
    expect(container.querySelector('button[aria-label="Pane settings"]')).toBeNull();
